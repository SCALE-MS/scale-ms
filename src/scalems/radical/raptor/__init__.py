...
async def raptor_input(
    *,
    ...
    worker_pre_exec: list,
    ranks_per_worker: int,
    cores_per_rank: int,
    gpus_per_rank: typing.SupportsFloat,
) -> _file.AbstractFileReference:
    """Provide the input file for a SCALE-MS Raptor script.

    Args:
        ranks_per_worker: number of processes (TaskDescription.ranks) for each Worker Task
        cores_per_rank: set the TaskDescription.cores_per_rank for the Worker Task(s)
        gpus_per_rank: fractional GPU count per Worker rank
    ...
    task_metadata = worker_requirements(
        pre_exec=worker_pre_exec,
